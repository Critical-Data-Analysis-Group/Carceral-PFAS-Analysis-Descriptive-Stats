...
For example, the following code iterates over every state FIPS code in the U.S., extracting the variables ```pct_Pop_NoCompDevic_ACS_15_19```, ```pct_Pop_w_BroadComp_ACS_15_19```, ```pct_Civ_unemp_16p_ACS_15_19``` at the tract level, and returning a data frame with each row representing a specific tract.
