...
prison_boundaries_block_census %>% 
  ...
  summarize() %>%
  select(Median_Age_ACS_15_19, TYPE) %>% 
...
prison_boundaries_block_census %>% 
  group_by(STATE, COUNTY, TRACTCE10,
           ...
           SECURELVL) %>% 
  summarize() %>%
  select(Median_Age_ACS_15_19, SECURELVL) %>% 
...
prison_boundaries_block_census %>% 
  group_by(STATE, COUNTY, TRACTCE10,
           block_group, pct_URBANIZED_AREA_POP_CEN_2010, FACILITYID) %>% 
  summarize() %>%
...
prison_boundaries_block_census %>% 
  group_by(STATE, COUNTY, TRACTCE10,
           block_group, pct_RURAL_POP_CEN_2010, FACILITYID) %>% 
  summarize() %>%
...
prison_boundaries_block_census %>%
  ...
  kbl(caption = "<span style = 'color:black;font-size:2.5vw; margin-left: 2%'><b>Percentage of Facilities in a Block Group that was Mostly Rural or Mostly Urban</b></span>") %>%
...
tibble("Population Density" = c("Urban", "Urban Cluster", "Rural"),
       "Means" = c(mean(prison_boundaries_block_census$pct_URBAN_CLUSTER_POP_CEN_2010, na.rm = TRUE),
                   mean(prison_boundaries_block_census$pct_URBANIZED_AREA_POP_CEN_2010, na.rm = TRUE),
                    mean(prison_boundaries_block_census$pct_RURAL_POP_CEN_2010, na.rm = TRUE))) %>% kbl()
...
prison_boundaries_block_census %>% 
  ...
  kbl(caption = "<span style = 'color:black;font-size:2.5vw;'><b>&emsp;  Mean and Median Percentages in Urbanized, Urban Cluster, and Rural Areas</b></span><span style = 'float:center'><br> &emsp; Data from the 2010 Census</span>") %>%
  kable_material(c("striped", "hover")) %>%
  row_spec(0, background = "#D9DFEE") 
...
# percentage > 50 % urbanized and percentage where < 50 % urbanized 
