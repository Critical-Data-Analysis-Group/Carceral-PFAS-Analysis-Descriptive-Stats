...
prison_boundaries_block_census %>% 
  ...
  kbl(caption = "<span style = 'color:black;font-size:2vw;'><b>  Mean and Median Percentages in Urbanized, Urban Cluster, and Rural Areas</b></span><span style = 'float:center'><i><span style = 'font-size: 1vw'><br> Data from the 2010 Census</i></span>") %>%
...
prison_boundaries_block_census %>% 
  ...
  group_by(Density, SECURELVL) %>%
    mutate(Mean = mean(Median_Age_ACS_15_19, na.rm =TRUE),
           Median = median(Median_Age_ACS_15_19, na.rm = TRUE),
           SECURELVL = factor(SECURELVL, levels = c("JUVENILE", "MINIMUM", "MEDIUM",
                                                    "MAIMUM", "CLOSE", "NOT AVAILABLE"))) %>%
    filter(!is.na(Density)) %>% 
  ggplot(aes(x = Median_Age_ACS_15_19 )) + 
