...
prison_boundaries_block_census %>% 
  group_by(STATE, COUNTY, TRACTCE10,
           block_group, pct_URBANIZED_AREA_POP_CEN_2010) %>% summarize() %>% glimpse()
  mutate(Median = median(pct_URBANIZED_AREA_POP_CEN_2010, na.rm = TRUE),
         Mean = mean(pct_URBANIZED_AREA_POP_CEN_2010, na.rm = TRUE)) %>%
  select(pct_URBANIZED_AREA_POP_CEN_2010, Median, Mean) %>% 
  ggplot(aes(x = pct_URBANIZED_AREA_POP_CEN_2010 )) + 
...
prison_boundaries_block_census %>% 
  ...
  labs(y = "Number of Facilities", 
       x = "Median Age",
       subtitle = "2015 – 2019 5-year ACS sample data & 2010 Census Data",
       title = "Distribution of Median Age in Block Groups Where Facilities are Located\n by Population Density Category and Facility Type") +
  theme(text = element_text(family = "Optima"),
        plot.subtitle = element_text(face = "italic", hjust = .5),
        plot.title = element_text(face = "bold", hjust = 0.5))
...
> As defined in the [documentation](https://www2.census.gov/adrm/PDB/2021/2021_Block_Group_PDB_Documentation.pdf), ```Number of people who have no health insurance coverage, public or private, in the ACS population```. To find the percentage lacking health insurance, we can divide by the total population, ```Tot_Population_ACS_15_19```. 
